...
# ══════════════════════════════════════════════════════════════════════════════
#  HƯỚNG DẪN CHẤM — PHẦN III TRẢ LỜI NGẮN (chỉ dùng trong export-similar-docx)
#  Bảng 2 cột gọn: [Câu | Đáp án]  — không có cột Ý a/b/c/d thừa
# ══════════════════════════════════════════════════════════════════════════════
def build_answer_key_tln(doc, questions: list, diem_moi_cau, section_num: int,
                         label: str, font_size_head=None, font_size_table=None):
    """
    Bảng đáp án Phần III Trả lời ngắn — chỉ 2 cột: Câu | Đáp án.
    Chỉ được gọi từ export_similar_docx. Không ảnh hưởng các tab khác.
    font_size_head / font_size_table cho phép override cỡ chữ cục bộ.
    """
    if not questions:
        return
    fh = font_size_head  if font_size_head  else SIZE_HEAD
    ft = font_size_table if font_size_table else SIZE_TABLE

    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(8)
    add_run_with_font(p,
        f"PHẦN {_roman(section_num)}. {label} (Mỗi câu đúng được {diem_moi_cau} điểm)",
        bold=True, font_size=fh
    )
    ...
    for i, h in enumerate(headers):
        cell = table.cell(0, i)
        set_cell_bg(cell, BG_HEADER)
        add_cell_rich_text(cell, h, bold=True,
                           align=WD_ALIGN_PARAGRAPH.CENTER, font_size=ft)
    ...
    for r_idx, q in enumerate(questions):
        ...
        add_cell_rich_text(row.cells[0], str(r_idx + 1), bold=True,
                           align=WD_ALIGN_PARAGRAPH.CENTER, font_size=ft)
        ans = clean_ans(get_value(q, ["dapAnDung", "answer", "dapAn"], ""))
        add_cell_rich_text(row.cells[1], ans,
                           align=WD_ALIGN_PARAGRAPH.CENTER, font_size=ft)
...
# ══════════════════════════════════════════════════════════════════════════════
#  ENDPOINT: POST /api/parse-exam-file  (Parse Word/PDF → text + ảnh)
# ══════════════════════════════════════════════════════════════════════════════
@app.post("/api/parse-exam-file")
async def parse_exam_file(file: UploadFile = File(...)):
    """
    Nhận file Word (.docx) hoặc PDF từ client.
    - Word: Dùng python-docx để lấy text + ảnh base64
    - PDF: Dùng pdfplumber để lấy text, pdfminer cho ảnh
    Trả về: { text: str, images: list, pageCount: int, fileName: str }
    """
    import io, base64
    ...
    if ext == "docx":
        ...
        return JSONResponse(
            status_code=400,
            content={"error": f"Chỉ hỗ trợ .docx và .pdf. File bạn tải lên: '{filename}'"}
        )


async def _parse_docx(content: bytes, filename: str) -> dict:
    """Parse Word .docx → { text, images, pageCount, fileName }"""
    import io, base64
    ...
    # Helper: xử lý 1 paragraph (có thể chứa run text + inline image)
    def process_para(para):
        text_parts = []
        for run in para.runs:
            # Kiểm tra xem run có chứa ảnh không (w:drawing / w:pict)
            xml = run._r.xml
            if 'graphicData' in xml or 'imagedata' in xml or 'blip' in xml:
                # Lấy image từ relationship
                for rel in doc.part.rels.values():
                    if "image" in rel.reltype:
                        try:
                            img_bytes = rel.target_part.blob
                            img_ext = rel.target_part.content_type.split("/")[-1].replace("jpeg", "jpg")
                            b64 = base64.b64encode(img_bytes).decode()
                            img_id = f"img_{img_counter[0]}"
                            img_counter[0] += 1
                            # Tránh duplicate
                            if not any(i["id"] == img_id for i in images):
                                images.append({
                                    "id": img_id,
                                    "base64": f"data:image/{img_ext};base64,{b64}",
                                    "mime": f"image/{img_ext}"
                                })
                            text_parts.append(f"[HÃNH ẢNH: {img_id}]")
                        except:
                            pass
            else:
                if run.text:
    ...
    # Lấy tất cả ảnh trong document (kể cả floating)
    seen_rels = set()
    for rel in doc.part.rels.values():
        if "image" in rel.reltype:
            try:
                img_bytes = rel.target_part.blob
                img_ext = rel.target_part.content_type.split("/")[-1].replace("jpeg", "jpg")
                rel_key = base64.b64encode(img_bytes[:64]).decode()  # fingerprint
    ...
    return {
        "text": full_text,
        "images": images,
        "pageCount": None,  # docx không có khái niệm trang rõ ràng
        "fileName": filename,
        "charCount": len(full_text)
    }


async def _parse_pdf(content: bytes, filename: str) -> dict:
    """Parse PDF → { text, images, pageCount, fileName }"""
    import io, base64
    try:
        import pdfplumber
    except ImportError:
        return JSONResponse(
            status_code=500,
            content={"error": "pdfplumber chưa cài. Chạy: pip install pdfplumber"}
        )
    ...
    try:
        with pdfplumber.open(io.BytesIO(content)) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                if page_text.strip():
                    text_pages.append(f"--- Trang {page_num} ---\n{page_text}")

                # Lấy ảnh từ trang PDF
                for img_info in page.images:
                    try:
                        # pdfplumber cung cấp bbox, cần crop và save
                        ...
                        print(f"[parse_pdf] Lỗi lấy ảnh trang {page_num}: {e}")

    except Exception as e:
        return JSONResponse(
            status_code=500,
            content={"error": f"Lỗi đọc PDF: {str(e)}"}
        )
...
# ══════════════════════════════════════════════════════════════════════════════
#  ENDPOINT: POST /api/render-typst  (Biên dịch Typst → SVG — chỉ cho Bước 6)
# ══════════════════════════════════════════════════════════════════════════════
@app.post("/api/render-typst")
async def render_typst(request: Request):
    """
    Nhận mã Typst, biên dịch sang SVG bằng typst CLI và trả về SVG string.
    Chỉ sử dụng bởi Step6_SimilarExam.jsx — không ảnh hưởng tính năng khác.
    Body: { "code": "... mã typst ..." }
    Trả về: { "success": true, "svg": "<svg>...</svg>" }
          | { "success": false, "error": "..." }
    """
    data = await request.json()
    typst_code = data.get("code", "").strip()
    if not typst_code:
        return JSONResponse({"success": False, "error": "Mã Typst trống."})

    with tempfile.TemporaryDirectory() as tmpdir:
        typ_path = os.path.join(tmpdir, "figure.typ")
        ...
        try:
            import typst
            typst.compile(typ_path, output=svg_path, format="svg")
        except ImportError:
            return JSONResponse({
                "success": False,
                "error": "Chưa cài đặt package python `typst`. Hãy chạy: pip install typst"
            })
        except Exception as e:
            err_msg = str(e).strip()
            return JSONResponse({"success": False, "error": err_msg[:500]})

        # Typst có thể xuất ra figure.svg hoặc figure1.svg (trang 1)
        actual_svg = svg_path
        if not os.path.exists(actual_svg):
            # Thử tên có hậu tố trang
            candidate = os.path.join(tmpdir, "figure1.svg")
            if os.path.exists(candidate):
                actual_svg = candidate
            else:
                return JSONResponse({"success": False, "error": "Không tìm thấy file SVG đầu ra."})

        with open(actual_svg, "r", encoding="utf-8") as f:
            svg_content = f.read()
...
# ══════════════════════════════════════════════════════════════════════════════
#  ENDPOINT: POST /api/export-similar-docx  (Xuất đề tương tự ra Word — Bước 6)
#  Chỉ sử dụng bởi Step6_SimilarExam.jsx — không ảnh hưởng tính năng khác.
# ══════════════════════════════════════════════════════════════════════════════

# ── Helper nội bộ: thêm run với hỗ trợ chỉ số trên/dưới Unicode ─────────────
_SUP_MAP = {
    '⁰':'0','¹':'1','²':'2','³':'3','⁴':'4',
    '⁵':'5','⁶':'6','⁷':'7','⁸':'8','⁹':'9',
    '⁺':'+','⁻':'-','⁼':'=','⁽':'(','⁾':')'
...
def _clean_pdf_text(text: str) -> str:
    """Xóa ký tự rác PDF (□, ■...). Dùng riêng trong /api/export-similar-docx."""
    if not text:
        return text
    return _BOX_RE.sub('', text)

def _add_runs_with_scripts(para, text: str, bold=False, italic=False,
                           font_size=None, font_name=FONT_NAME):
    """
    Thêm text vào paragraph, tự động tách superscript/subscript Unicode
    thành các run định dạng Word thực sự.
    Dùng riêng trong /api/export-similar-docx.
    """
    if font_size is None:
        font_size = SIZE_BODY

    # Tách thành các đoạn: thường, sup, sub
    i = 0
    while i < len(text):
        # Tìm vị trí sớm nhất của sup/sub
        sup_m = _SUP_RE.search(text, i)
        sub_m = _SUB_RE.search(text, i)

        # Chọn match gần nhất
        next_sup = sup_m.start() if sup_m else len(text)
        next_sub = sub_m.start() if sub_m else len(text)
        next_special = min(next_sup, next_sub)

        # Phần text thường trước
        if next_special > i:
        ...
        # Phần sup hoặc sub
        if next_special == next_sup and (sub_m is None or next_sup <= next_sub):
            m = sup_m
            char_map = _SUP_MAP
            is_sup = True
        else:
        ...
        conv = ''.join(char_map.get(c, c) for c in m.group())
        run = para.add_run(conv)
        run.font.superscript = is_sup
        run.font.subscript   = not is_sup
        run.font.name = font_name
        if font_size:
            run.font.size = font_size
        rPr = run._r.get_or_add_rPr()
...
def auto_wrap_latex(text: str) -> str:
    """
    Tự động tìm các đoạn công thức LaTeX/số mũ chưa được bọc trong $...$ và bọc chúng lại.
    Chỉ dùng riêng cho tab Tạo đề tương tự.
    """
    if not text:
        return text
    if '$' in text:
    ...
    # Các mẫu LaTeX/Math cực kỳ an toàn để tự động bọc:
    # 1. Các lệnh LaTeX bắt đầu bằng gạch chéo ngược (ví dụ: \text{...}, \pi, \frac{a}{b}, \times)
    # 2. Số mũ (ví dụ: x^2, R^2, cm^3, m^2, y^{n+1})
    pattern = re.compile(
        r'('
...
def _add_latex_para_similar(para, text: str, bold=False, italic=False,
                             font_size=None, mode="math"):
    """
    Phân tích text có thể chứa $...$ / $$...$$, xử lý LaTeX → OMML
    và text thường → run với chỉ số trên/dưới.
    Dùng riêng trong /api/export-similar-docx.
    """
    if not text:
        return
    ...
    if mode == "normal":
        # Thay thế một số đơn vị / ký hiệu LaTeX thông dụng để dễ đọc trong chế độ môn thường
        replacements = {
            r"\text{ cm}^3": " cm³",
            r"\text{ m}^2": " m²",
            r"\text{cm}^3": "cm³",
    ...
    if mode in ("math", "latex"):
        text = auto_wrap_latex(text)

    if mode != "math":
        _add_runs_with_scripts(para, text, bold=bold, italic=italic, font_size=font_size)
        return
    ...
    for part in parts:
        ...
        if part.startswith('$'):
            ...
            if omml is not None:
                ...
                _add_runs_with_scripts(para, part, italic=True, font_size=font_size)
        else:
            _add_runs_with_scripts(para, part, bold=bold, italic=italic,
                                   font_size=font_size)


def _insert_image_similar(doc, q: dict, image_map: dict):
    """
    Chèn hình ảnh vào doc cho câu hỏi q trong /api/export-similar-docx:
      - svgCode → chuyển SVG thành PNG tạm qua cairosvg (nếu có) hoặc bỏ qua
      - imageRef → giải mã base64 từ image_map
      - biểu đồ/đồ thị → generate_graph Matplotlib
    """
    ...
    loai = hinh_anh.get("loai", "")
    svg_code = hinh_anh.get("svgCode", "")
    image_ref = hinh_anh.get("imageRef", "")

    inserted = False

    # Ưu tiên 1: svgCode (Typst/SVG do AI sinh hoặc GV vẽ thủ công)
    if svg_code and not inserted:
        try:
            from svglib.svglib import svg2rlg
            from reportlab.graphics import renderPM
            import io
            
            # svglib cần input là file hoặc file-like string có chứa root <svg>
            ...
            # svglib chưa cài — ghi chú thay thế
            p = doc.add_paragraph()
            _add_runs_with_scripts(p, "[Hình SVG — cần cài svglib và reportlab trên server để xuất ảnh]",
                                   italic=True, font_size=SIZE_SMALL)
            inserted = True
        except Exception as e:
            p = doc.add_paragraph()
            _add_runs_with_scripts(p, f"[Lỗi render SVG: {e}]",
                                   italic=True, font_size=SIZE_SMALL)
            inserted = True

    # Ưu tiên 2: imageRef → ảnh gốc từ file tải lên
    if image_ref and not inserted:
        img_data = image_map.get(image_ref)
        if img_data:
            try:
                raw_b64 = img_data.get("base64", "")
                ...
                p = doc.add_paragraph()
                _add_runs_with_scripts(p, f"[Lỗi chèn ảnh: {e}]",
                                       italic=True, font_size=SIZE_SMALL)
                inserted = True

    # Ưu tiên 3: biểu đồ / đồ thị Matplotlib
    if not inserted and loai and loai != "khac":
        try:
            result = generate_graph(hinh_anh)
            if result.get("success") and result.get("filePath"):
                doc.add_picture(result["filePath"], width=Inches(4))
            ...
            p = doc.add_paragraph()
            _add_runs_with_scripts(p, f"[Lỗi vẽ biểu đồ: {e}]",
                                   italic=True, font_size=SIZE_SMALL)


@app.post("/api/export-similar-docx")
async def export_similar_docx(request: Request):
    """
    Xuất đề thi tương tự (Bước 6) ra Word với:
      - Làm sạch ký tự rác PDF (□ ■ …)
      - Chỉ số trên/dưới Unicode (³ ² ₂ ₃) → run superscript/subscript thực sự
      - Chèn ảnh từ imageRef / svgCode / biểu đồ
      - Xuất hướng dẫn chấm ở trang 2
    Chỉ sử dụng bởi Step6_SimilarExam.jsx.
    Body: {
      "questions":  [ {...} ],   // danh sách câu hỏi đề tương tự (đã sinh)
      "examTitle":  "ĐỀ THI TƯƠNG TỰ - MÃ ĐỀ 101",
      "examHeader": { soGD, truong, kyThi, monHoc, thoiGian, namHoc },
      "examConfig": { diemMoiCauP1, diemMoiYP2, diemMoiYP3 },
      "imageMap":   { "<id>": { base64, mime } }   // ảnh gốc từ file tải lên
    }
    """
    import docx as docx_module
    ...
    exam_title    = data.get("examTitle",  "ĐỀ THI TƯƠNG TỰ")
    exam_header   = data.get("examHeader", {})
    exam_config   = data.get("examConfig", {})
    image_map     = data.get("imageMap",   {})
    mode          = data.get("mode", "math")

    # ── Cỡ chữ cục bộ cho tab Tạo đề tương tự (13pt) ────────────────────────
    # Dùng biến cục bộ để KHÔNG ảnh hưởng hằng số toàn cục SIZE_BODY (12pt)
    # dùng bởi các tab khác (/api/export-docx, /api/export-docx-with-images)
    SIM_SIZE_TITLE = Pt(15)
    SIM_SIZE_HEAD  = Pt(14)
    SIM_SIZE_BODY  = Pt(13)
    SIM_SIZE_TABLE = Pt(11.5)
    SIM_SIZE_SMALL = Pt(11)

    diem_p1 = float(exam_config.get("diemMoiCauP1", 0.25))
    diem_p2 = float(exam_config.get("diemMoiYP2",   0.25))
    diem_p3 = float(exam_config.get("diemMoiYP3",   0.25))

    # ── Phân loại câu hỏi theo loaiCauHoi ─────────────────────────────────
    mcq        = [q for q in questions_all if q.get("loaiCauHoi") == 1]
    dung_sai   = [q for q in questions_all if q.get("loaiCauHoi") == 2]
    tra_loi_ngan = [q for q in questions_all if q.get("loaiCauHoi") == 3]
    tu_luan    = [q for q in questions_all if q.get("loaiCauHoi") == 4]
    ...
    p.paragraph_format.space_before = Pt(12)
    p.paragraph_format.space_after  = Pt(6)
    add_run_with_font(p, exam_title, bold=True, font_size=SIM_SIZE_TITLE)
    doc.add_paragraph()

    # ── Số thứ tự phần ──────────────────────────────────────────────────────
    sec = 1
    sec_mcq = sec if mcq else None;        sec += (1 if mcq else 0)
    sec_ds  = sec if dung_sai else None;   sec += (1 if dung_sai else 0)
    sec_tln = sec if tra_loi_ngan else None; sec += (1 if tra_loi_ngan else 0)
    sec_tl  = sec if tu_luan else None

    # ── Helper nội bộ: viết 1 câu hỏi vào doc ───────────────────────────────
    def _write_question_noidung(doc, q, label_str):
        """Viết dòng 'Câu X. <nội dung>' có LaTeX + chỉ số. Dùng SIM_SIZE_BODY (13pt)."""
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(6)
        p.paragraph_format.space_after  = Pt(3)
        add_run_with_font(p, label_str, bold=True, font_size=SIM_SIZE_BODY)
        noi_dung = _clean_pdf_text(clean_meta(
            get_value(q, ["noiDung", "noidung", "content"], "")
        ))
        _add_latex_para_similar(p, noi_dung, font_size=SIM_SIZE_BODY, mode=mode)

        # Bảng biểu
        bang_bieu = q.get("bangBieu")
        if bang_bieu and isinstance(bang_bieu, dict) and bang_bieu.get("data"):
            rows_data = bang_bieu["data"]
            ...
            for ri, row in enumerate(rows_data):
                for ci, cell_val in enumerate(row):
                    ...
                    if is_header:
                        set_cell_bg(cell, BG_HEADER)
                    add_cell_rich_text(cell, str(cell_val), bold=is_header,
                                       align=WD_ALIGN_PARAGRAPH.CENTER,
                                       font_size=SIZE_TABLE, mode=mode)

        # Hình ảnh
        _insert_image_similar(doc, q, image_map)
    ...
    if mcq and sec_mcq:
        ...
        p.paragraph_format.space_before = Pt(12)
        add_run_with_font(p,
            f"PHẦN {_roman(sec_mcq)}. Câu trắc nghiệm nhiều phương án lựa chọn",
            bold=True, font_size=SIM_SIZE_HEAD)

        for idx, q in enumerate(mcq):
            _write_question_noidung(doc, q, f"Câu {idx + 1}. ")

            labels = ["A", "B", "C", "D"]
            keys   = ["dapAnA", "dapAnB", "dapAnC", "dapAnD"]
            options = []
            for lbl, k in zip(labels, keys):
                val = str(q.get(k, "") or "").strip()
                if not val:
                    continue
                val = _clean_pdf_text(val)
                if mode in ("math", "latex"):
                    val = auto_wrap_latex(val)
                elif mode == "normal":
                    replacements = {
                        r"\text{ cm}^3": " cm³",
                        r"\text{ m}^2": " m²",
                    ...
                    for r_k, r_v in replacements.items():
                        val = val.replace(r_k, r_v)
                    val = val.replace("$$", "").replace("$", "")
                options.append((lbl, val))
            ...
            if all_short and len(options) == 4:
                ...
                for i, (lbl, val) in enumerate(options):
                    cell = tbl.cell(0, i)
                    add_cell_rich_text(cell, f"{lbl}. {val}", font_size=SIM_SIZE_BODY, mode=mode)
            else:
                for lbl, val in options:
                    po = doc.add_paragraph()
                    po.paragraph_format.left_indent = Cm(1)
                    po.paragraph_format.space_after  = Pt(2)
                    _add_latex_para_similar(po, f"{lbl}. {val}", font_size=SIM_SIZE_BODY, mode=mode)
            # Không chèn dòng trống — space_before trong _write_question_noidung đủ để cách câu
    ...
    if dung_sai and sec_ds:
        ...
        p.paragraph_format.space_before = Pt(12)
        add_run_with_font(p,
            f"PHẦN {_roman(sec_ds)}. Câu trắc nghiệm đúng sai",
            bold=True, font_size=SIM_SIZE_HEAD)

        for idx, q in enumerate(dung_sai):
            _write_question_noidung(doc, q, f"Câu {idx + 1}. ")
            for lbl, key in [("a", "yA"), ("b", "yB"), ("c", "yC"), ("d", "yD")]:
                val = _clean_pdf_text(str(q.get(key, "") or ""))
                if not val:
                    continue
                po = doc.add_paragraph()
                po.paragraph_format.left_indent = Cm(1)
    ...
    if tra_loi_ngan and sec_tln:
        ...
        p.paragraph_format.space_before = Pt(12)
        add_run_with_font(p,
            f"PHẦN {_roman(sec_tln)}. Câu trắc nghiệm trả lời ngắn",
            bold=True, font_size=SIM_SIZE_HEAD)

        for idx, q in enumerate(tra_loi_ngan):
            _write_question_noidung(doc, q, f"Câu {idx + 1}. ")
            # Không chèn dòng trống — space_before trong _write_question_noidung đủ để cách câu
    ...
    if tu_luan and sec_tl:
        ...
        p.paragraph_format.space_before = Pt(12)
        add_run_with_font(p,
            f"PHẦN {_roman(sec_tl)}. Câu hỏi tự luận",
            bold=True, font_size=SIM_SIZE_HEAD)

        for idx, q in enumerate(tu_luan):
            _write_question_noidung(doc, q, f"Câu {idx + 1}. ")
            y_parts = get_y_parts(q)
            for part in y_parts:
                if not part["y"]:
                ...
                po = doc.add_paragraph()
                po.paragraph_format.left_indent = Cm(1)
                po.paragraph_format.space_after  = Pt(3)
                label_text = f"{part['label']}) " if part["label"] else ""
                _add_latex_para_similar(po, label_text + _clean_pdf_text(clean_meta(part["y"])),
                                        bold=bool(label_text), font_size=SIM_SIZE_BODY, mode=mode)
            # Không chèn dòng trống — space_before trong _write_question_noidung đủ để cách câu

    # ── NGẮT TRANG → HƯỚNG DẪN CHẤM ────────────────────────────────────────
    from docx.oxml import OxmlElement as _OE_sim
    pb = doc.add_paragraph()
    pb_run = pb.add_run()
    br_pg = _OE_sim("w:br")
    br_pg.set(qn("w:type"), "page")
    pb_run._r.append(br_pg)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_before = Pt(0)
    p.paragraph_format.space_after  = Pt(12)
    add_run_with_font(p, "HƯỚNG DẪN CHẤM VÀ BIỂU ĐIỂM",
                      bold=True, font_size=SIZE_TITLE)

    if mcq and sec_mcq:
        build_answer_key_mcq(doc, mcq, diem_p1, sec_mcq)
    if dung_sai and sec_ds:
        build_answer_key_ds_or_tln(doc, dung_sai, diem_p2, sec_ds,
                                   "Trắc nghiệm Đúng/Sai")
    if tra_loi_ngan and sec_tln:
        build_answer_key_tln(doc, tra_loi_ngan, diem_p3, sec_tln,
                             "Trắc nghiệm Trả lời ngắn",
                             font_size_head=SIM_SIZE_HEAD,
                             font_size_table=SIM_SIZE_TABLE)
    if tu_luan and sec_tl:
        build_answer_key_tu_luan(doc, tu_luan, sec_tl)

    # ── Lưu & trả về ────────────────────────────────────────────────────────
    import tempfile as _tempfile
    filename = os.path.join(_tempfile.gettempdir(), f"export_similar_{uuid.uuid4().hex}.docx")
    try:
        doc.save(filename)
    except Exception as e:
        return JSONResponse({"error": f"Lỗi khi lưu file: {str(e)}"}, status_code=500)

    safe_title = re.sub(r'[^a-zA-Z0-9_\-]', '_', exam_title)[:40]
    return FileResponse(
        filename,
