...
if (brokenPartIndex !== -1) {
  // We need to find the start of the broken part where the diff deleted lines 731-771
  // Wait, let's look at the diff:
  //           spacing: { after: 150 }
  //         }));
  //       }
  // - [deleted tfHeaderRow etc]
  // - [deleted Part 3 start]
  ...
  // Find where the loop for totalSA ends
  //            spacing: { after: 120 }
  //          }));
  //        }
  //      }
  ...
  //    }
  //  }
  const endSearchStr = `answerKeyParagraphs.push(new Paragraph({ text: "", spacing: { after: 200 } }));\n      }\n    }`;
  const endIdx = c.indexOf(endSearchStr, brokenPartIndex);
  ...
  if (startIdx !== -1 && endIdx !== -1) {
    ...
    if (config.hasTraLoiNgan) {
      ...
      c = c.substring(0, startIdx) + newContent + c.substring(fullEndIdx);
