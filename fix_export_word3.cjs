...
const anchorStr = `        borders: standardBorders // BẬT VIỀN ĐẸP
          const slotData = examSlots[\`phan3_cau\${i + 1}\`];`;

const replacement = `        borders: standardBorders // BẬT VIỀN ĐẸP
      }));
...
if (c.includes(anchorStr)) {
  c = c.replace(anchorStr, replacement);
