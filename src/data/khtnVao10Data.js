...
export const generateKhtnVao10Matrix = () => {
  ...
  return KHTN_VAO10_SAMPLE_TOPICS.map((topic, index) => {
    ...
    for (let h = 0; h < (topic.nhieuLuaChon?.hieu || 0); h++) {
      indicatorMap[`nhieuLuaChon_hieu_${h}`] = { code: 'NT3', label: `I.${p1Counter++}` };
    }
    ...
    if (dsHieu >= 2 && dsVd >= 2) {
      ...
      dungSaiSubItems.push(
        { qNo, letter: 'a', lvl: 'hieu', label: `II.${qNo}a`, code: 'NT3' },
      ...
      indicatorMap[`dungSai_hieu_0`] = { code: 'NT3', label: `II.${qNo}a` };
