...
⚠️ PHÂN BỔ ĐIỂM: Tổng điểm các bước giải của mỗi câu phải khớp CHÍNH XÁC với số điểm được giao. KHÔNG ĐƯỢC vượt quá hoặc thiếu.\n\n`;
    ...
    // Helper: auto-suy mã HH/TD từ YCCĐ khi indicatorMap rỗng
  ...
  const handleBulkPush = () => {
    ...
    const emptySlotsLoai1 = availableSlots.filter(s => s.loai === 1 && !s.daDien);
    const emptySlotsLoai2 = availableSlots.filter(s => s.loai === 2 && !s.daDien);
    const emptySlotsLoai3 = availableSlots.filter(s => s.loai === 3 && !s.daDien);
    const emptySlotsLoai4 = availableSlots.filter(s => s.loai === 4 && !s.daDien);

    const pushGroup = (questions, emptySlots) => {
      let qIndex = 0;
    ...
    // ── ĐẨY LOẠI 1, 2, 3 NHƯ CŨ ──
    pushGroup(qsLoai1, emptySlotsLoai1);
    pushGroup(qsLoai2, emptySlotsLoai2);
    pushGroup(qsLoai3, emptySlotsLoai3);

    // ── ĐẨY LOẠI 4 (TỰ LUẬN): MỖI CÂU AI → 1 SLOT RIÊNG (khớp Step4) ──
    {
      let tlIdx = 0;
      for (const slot of emptySlotsLoai4) {
        if (tlIdx >= qsLoai4.length) break;
        const q = qsLoai4[tlIdx];
        const mappedData = mapTL4Draft(q);
        if (q.hinhAnh) mappedData.hinhAnh = q.hinhAnh;
