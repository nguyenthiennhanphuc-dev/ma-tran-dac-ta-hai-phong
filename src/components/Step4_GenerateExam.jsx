...
export default function Step4_GenerateExam() {
  ...
  // 2. Phần Đúng/Sai — MỖI CÂU LUÔN CÓ 4 Ý: a) NB, b) TH, c) VD, d) VDC
  const tfQuestions = (() => {
    // Thu thập ý theo từng mức độ
    const byLevel = { biet: [], hieu: [], vanDung: [], vanDungCao: [] };
    matrix.forEach(topic => {
      (topic.donViKienThuc || []).forEach(dv => {
        const ds = dv.dungSai || {};
        ['biet', 'hieu', 'vanDung', 'vanDungCao'].forEach(key => {
          ...
          for (let i = 0; i < count; i++) {
            byLevel[key].push({ topic: topic.tenChuDe || 'Chưa nhập tên chủ đề' });
          }
        });
      });
    });
    ...
    // Số câu = min của 4 mức (đảm bảo mỗi câu đủ 4 ý)
    const numQ = Math.max(byLevel.biet.length, byLevel.hieu.length, byLevel.vanDung.length, byLevel.vanDungCao.length);
    const allQuestions = [];
    for (let i = 0; i < numQ; i++) {
      const chunk = [];
      ['biet', 'hieu', 'vanDung', 'vanDungCao'].forEach(key => {
        if (i < byLevel[key].length) {
          chunk.push({ topic: byLevel[key][i].topic, level: levelLabels[key] });
        }
      });
      if (chunk.length > 0) allQuestions.push(chunk);
    }
    return allQuestions;
  })();
